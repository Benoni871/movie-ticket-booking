...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, PosterCarouselComponent],
  template: `
    <!-- Hero carousel -->
    @if (movies().length > 0) {
      <div class="mb-10">
          ...
          <div class="group block card overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative">
            ...
            <a [routerLink]="['/movies', m.id, 'book']" class="block">
              ...
              <div class="aspect-[2/3] bg-slate-900 overflow-hidden relative">
                ...
                <!-- Top-right runtime -->
                <span class="absolute top-2 right-2 inline-flex items-center rounded-full bg-black/60 backdrop-blur-sm text-white px-2 py-0.5 text-[10px] font-semibold">
                  ⏱ {{ m.durationMins }} min
                </span>

                <!-- Hover Book CTA -->
                <div class="absolute bottom-0 left-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div class="inline-flex items-center justify-center w-full rounded-lg bg-indigo-600/95 text-white text-sm font-semibold py-2 shadow-lg backdrop-blur-sm">
                    Book Tickets →
                  </div>
                </div>
              </div>
            ...
            <div class="p-4">
              ...
              <div class="flex items-center justify-between mt-1.5">
                ...
                <span class="inline-flex items-center gap-1 text-xs font-semibold"
                      [class.text-amber-500]="(m.reviewCount ?? 0) > 0"
              ...
              @if (embedUrl(m)) {
                <button type="button" (click)="openTrailer(m)"
                        class="mt-3 w-full inline-flex items-center justify-center gap-1 rounded-lg border border-rose-200 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-semibold py-1.5 transition">
                  ▶ Watch Trailer
                </button>
              }
            </div>
    ...
    @if (trailerTarget()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md"
        ...
        <div class="relative max-w-4xl w-full" (click)="$event.stopPropagation()">
          ...
          <div class="mt-3 text-center text-white/80 text-sm font-medium">
            🎬 {{ trailerTarget()!.title }}
          </div>
        </div>
      </div>
...
export class MovieGridComponent implements OnInit {
  ...
  ngOnInit() {
    this.theaterService.getAll().subscribe(list => this.theaters.set(list));
  ...
  reload() {
    ...
    this.movieService.getAll(tid != null ? { theaterId: tid } : undefined).subscribe({
